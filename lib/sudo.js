...
function normalize(number) {
    // WhatsApp can provide JIDs such as 923008872807:0@s.whatsapp.net.
    // Strip the device suffix before normalizing the phone number.
    let raw = String(number || '').trim().split('@')[0].split(':')[0];
...
function syncConfigOwners() {
    ...
    if (!config.OWNER_NUMBER.some(n => normalize(n) === PRIMARY_OWNER)) {
        config.OWNER_NUMBER.push(PRIMARY_OWNER);
    }
    for (const number of sudoNumbers) {
        if (!config.OWNER_NUMBER.some(n => normalize(n) === number)) config.OWNER_NUMBER.push(number);
    }
}
...
async function removeSudo(number) {
    ...
    if (Array.isArray(config.OWNER_NUMBER)) {
        config.OWNER_NUMBER = config.OWNER_NUMBER.filter(n => normalize(n) !== clean || normalize(n) === PRIMARY_OWNER);
    }
    syncConfigOwners();
